...
const replacementHtml = htmlWidget + `\n    if (_activeLessonTab === 'writing' && item) {`;

if (!html.includes('Intelligent Translator Writing Pad')) {
  // Be careful replacing this because there's also the title header `if (_activeLessonTab === 'writing') html += '<h3 class="study-lead"...`
  // The exact string to replace is inside the `html += '<div style="background:var(--card2);...` block.
  ...
  const exactTarget = "    if (_activeLessonTab === 'writing') {\n      html += '<div style=\"font-weight:700; color:var(--gold2); font-size:16px; margin-bottom:8px;\">'";
  const exactReplacement = htmlWidget + "\n    if (_activeLessonTab === 'writing' && item) {\n      html += '<div style=\"font-weight:700; color:var(--gold2); font-size:16px; margin-bottom:8px;\">'";
  
  html = html.replace(exactTarget, exactReplacement);
}
