#!/usr/bin/env python3
"""
VilfinTV Blog Intelligence Hub — Daily Top Mutual Funds News Feed Updater
========================================================================
Builds data/top_mutual_funds.json — the auto-live half of the Top Mutual Funds board on
blog_intelligence_hub.html. Pulls "top mutual funds" news
...
The accurate, structured Top 5 Mutual Funds list is managed
separately from the VilfinTV Console and stored in config.json (top_mutual_funds);
the frontend merges both and removes any fund if its rank falls below top 5.

Output: data/top_mutual_funds.json
"""
...
SESSION.headers.update({
    "User-Agent": "Mozilla/5.0 (compatible; VilfinTV-TopMFBot/1.0; +https://vilfin-tv.github.io/MultiScreener)",
...
# Per-country feed lists — "top mutual funds" focus.
COUNTRY_FEEDS: dict[str, list[dict[str, str]]] = {
    "us": [
        _gn("top performing mutual funds 2026", "us"),
        _gn("best mutual funds to invest", "us"),
        _gn("mutual fund performance market", "us"),
    ],
    "japan": [
        _gn("おすすめ 投資信託 ランキング", "jp"),
        _gn("好成績 投資信託 ファンド 2026", "jp"),
        _gn("人気 投資信託 利回り", "jp"),
    ],
    "india": [
        {"url": "https://www.moneycontrol.com/rss/mfnews.xml", "source": "Moneycontrol MF"},
        _gn("top performing mutual funds India 2026", "in"),
        _gn("best mutual funds SIP returns India", "in"),
        _gn("mutual fund nav performance India", "in"),
    ],
}
...
# Only keep items that look like mutual fund news (avoids generic market noise)
KEYWORDS = re.compile(
    r"\b(mutual fund|mutual funds|投資信託|ファンド|ランキング|好成績|おすすめ|sip|nav|returns|performance|best funds|top funds)\b",
    re.IGNORECASE,
)
