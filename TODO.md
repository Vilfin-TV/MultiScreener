...
## ⏳ PENDING WORK
1. **IPTV — full Netflix/Fox row layout** (only the hero is done). Restructure the player/browse screen into hero + horizontal category rows of channel cards with hover‑scale. Risky rewrite of working player JS — do as its own careful phase, validate, don't break playback.
2. **IPTV play‑test** — needs ONE provider configured with a **real licensed M3U URL** first (Console → IPTV → Add provider). Then verify a channel actually plays (HLS path) and EPG loads. Cannot be done until the user adds a licensed source.
