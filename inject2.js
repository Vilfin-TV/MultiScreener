...
const css = `
.trans-widget { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 16px; margin-bottom: 24px; backdrop-filter: blur(10px); box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1); }
.trans-controls { display: flex; gap: 12px; align-items: center; margin-bottom: 16px; flex-wrap: wrap; }
.trans-select { flex: 1; min-width: 120px; background: var(--card); color: var(--text); border: 1px solid var(--border); padding: 10px; border-radius: 8px; font-size: 15px; outline: none; }
.trans-swap-btn { background: var(--blue); color: #fff; border: none; border-radius: 8px; padding: 10px 16px; font-weight: bold; cursor: pointer; transition: opacity 0.2s; }
.trans-swap-btn:hover { opacity: 0.8; }
.trans-body { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
@media(max-width: 768px) { .trans-body { grid-template-columns: 1fr; } }
.trans-box { background: var(--card2); border: 1px solid var(--border2); border-radius: 8px; display: flex; flex-direction: column; overflow: hidden; }
.trans-input, .trans-output { width: 100%; height: 120px; padding: 16px; font-size: 16px; color: var(--text); overflow-y: auto; box-sizing: border-box; background: transparent; border: none; outline: none; font-family: inherit; resize: none; }
.trans-actions { display: flex; justify-content: flex-end; padding: 8px 12px; background: rgba(0,0,0,0.2); border-top: 1px solid var(--border2); }
.trans-action-btn { background: transparent; color: var(--gold2); border: 1px solid var(--gold2); padding: 6px 12px; border-radius: 4px; font-size: 14px; cursor: pointer; display: flex; align-items: center; gap: 6px; }
.trans-action-btn:hover { background: var(--gold2); color: var(--bg); }
`;
if (!html.includes('.trans-widget {')) html = html.replace('</style>', css + '\n</style>');
...
`;
if (!html.includes('doTranslation() {')) html = html.replace('</script>\n\n</body>', js + '\n</script>\n\n</body>');
...
`;
html = html.replace("    if (_activeLessonTab === 'writing') {\n      html += '<div style=\"font-weight:700; color:var(--gold2); font-size:16px; margin-bottom:8px;\">' + esc(item.title) + '</div>'", widget);
html = html.replace("    if (_activeLessonTab === 'writing') {\r\n      html += '<div style=\"font-weight:700; color:var(--gold2); font-size:16px; margin-bottom:8px;\">' + esc(item.title) + '</div>'", widget);
