function renderItems(listEl, items) {
  ...
  if (layout === 'hero-list') {
    ...
    const listHtml = subItems.slice(3, 11).map(item => {
      const d = _d(item);
    ...
    const h = _d(all[0]);
    const heroHtml = `
      <div class="mano-hero" ${_ca(h)} style="${!h.imgSrc ? 'min-height:auto;' : 'aspect-ratio:4/3;'}">
    ...
    const subHtml = subItems.slice(0, 11).map(item => {
      const d = _d(item);
      return `
    ...
    html = `<div class="mano-left" style="align-self:start;position:sticky;top:20px;">${heroHtml}</div><div class="mano-right">${subHtml}</div>`;
