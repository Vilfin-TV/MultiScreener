const fs = require('fs');
let content = fs.readFileSync('scratch/renderItems2.js', 'utf8');

// 1. Increase limit to 15 (max items fetched)
content = content.replace('const all = items.slice(0, 12);', 'const all = items.slice(0, 15);');
...
// 6. Fix manorama balancing (distribute subItems evenly to eliminate empty space)
...
fs.writeFileSync('scratch/renderItems3.js', content);
