...
// 1. Replace the CSS for bn-card-title to remove truncation
const newCss = `.bn-card-title {
  font-size: 15px;
  font-weight: 700;
  color: var(--text);
  line-height: 1.4;
...
content = content.replace(/\.bn-card-title\s*\{[\s\S]*?\}/, newCss);

// 2. Replace renderBreakingNews function entirely
const newRenderFn = `
let breakingNewsData = null;
...
content = content.replace(/function renderBreakingNews\(items\) \{[\s\S]*?(?=\n\/\* ═══════════════════════════════════════════════════\n   RENDER: CATEGORIES)/, newRenderFn);

// 3. Fix the caller in init
content = content.replace(/if \(bnData && bnData\.length > 0\) \{/, 'if (bnData) {');

fs.writeFileSync('news.html', content);
console.log('Successfully patched news.html UI for breaking news tabs');
