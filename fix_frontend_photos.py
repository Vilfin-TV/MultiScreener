...
def update_frontend():
    ...
    # Find the kana function
    start_kana = content.find('function getKanaSvgVisual(romaji, wordStr) {')
    end_kana = content.find('var _activeKanaType =', start_kana)
    
    old_kana = content[start_kana:end_kana]
    
    new_kana = """// Setup CF_R2_URL mapping
const CF_R2_URL = 'https://pub-yourbucket.r2.dev';
...
"""
    content = content.replace(old_kana, new_kana)
    
    # 2. Update Kanji logic in renderKanjiDetails
    start_kanji = content.find("const detailsHtml = '<div class=\"kana-details-header\">'")
    end_kanji = content.find("document.getElementById('kanji-details-box').innerHTML = detailsHtml;", start_kanji)
    
    old_kanji = content[start_kanji:end_kanji]
    
    new_kanji = """const imgUrl = `${CF_R2_URL}/kanji/${kanjiChar}.jpg`;
  ...
  """
    ...
    content = content.replace(old_kanji, new_kanji)
